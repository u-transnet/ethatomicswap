...
try {
    atomicSwap.Initiate(7200, "0x" + secret.hashedSecret, AppConfig.hosts[0].defaultWallet, 100).then(function (result) {
        console.log("Generated initial transaction: " + result);
    }).catch(function(err) {
        console.log(err);
    });

    atomicSwap.Participate(7200, "0x" + secret.hashedSecret, AppConfig.hosts[0].defaultWallet, 100).then(function (result) {
